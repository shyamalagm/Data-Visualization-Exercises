...
CHIS <- read.dta("C:/Users/User/Desktop/R_Practice/Exercise_5_CHIS/ADULT.dta")
dim(CHIS)

library(dplyr)
adult <- CHIS %>%
  select(rbmi, bmi_p, racehpr2, srsex, srage_p, marit2, ab1, astcur, ab51, povll) %>%
  filter(racehpr2 %in% c("LATINO", "ASIAN", "WHITE", "AFRICAN AMERICAN"))
...
####Explore the dataset with summary and glimpse
...
# Histogram for Age colored by BMI
...
ggplot(adult, aes (x = SRAGE_P, fill= factor(RBMI))) +
  geom_histogram(binwidth = 1) +
  fix_strips + BMI_fill + facet_grid(RBMI ~ .) + 
  theme_classic()
